...
export class Store {
	...
	constructor({
		...
		initAnnotationStore: IAnnotationStore[];
	}) {
		this.pdfViewerApplication = PDFViewerApplication;
		this.annotationStore = new Map(
			initAnnotationStore.map((annotation) => [
				annotation.id,
				annotation,
			]),
		);
	...
	public restoreAnnotationStore(annotations: IAnnotationStore[]) {
		annotations.forEach((annotation) => {
			this.annotationStore.set(annotation.id, annotation);
		});
	}
	...
	public save(
		...
		this.pdfViewerApplication.pdfDocument.annotationStorage.setValue(
			`${PDFJS_INTERNAL_EDITOR_PREFIX}${id}`,
	...
	public update(id: string, updates: Partial<IAnnotationStore>) {
		if (this.annotationStore.has(id)) {
			...
			if (existingAnnotation) {
				...
				this.pdfViewerApplication.pdfDocument.annotationStorage.setValue(
					`${PDFJS_INTERNAL_EDITOR_PREFIX}${id}`,
					updates.pdfjsAnnotationStorage,
				);
			}
		} else {
	...
	public delete(id: string): void {
		if (this.annotationStore.has(id)) {
			...
			this.pdfViewerApplication.pdfDocument.annotationStorage.remove(
				`${PDFJS_INTERNAL_EDITOR_PREFIX}${id}`,
			);
		} else {
			console.warn(`Annotation with id ${id} not found.`);
	...
	public async resetAnnotationStorage(): Promise<void> {
		const annotationStorage =
			this.pdfViewerApplication.pdfDocument.annotationStorage;
		...
		this.annotationStore.forEach(async (annotation, id) => {
			if (annotation.content && annotation.content.image) {
				...
				annotationStorage.setValue(
					`${PDFJS_INTERNAL_EDITOR_PREFIX}${annotation.id}`,
					annotation.pdfjsAnnotationStorage,
				);
			}
		});
	}
