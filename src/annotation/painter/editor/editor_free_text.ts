...
export class EditorFreeText extends Editor {
	 ...
	 * @param pos 相对位置坐标
	 */
	private addInputAreaEventListeners(
		inputArea: HTMLTextAreaElement,
		scaleY: number,
		...
		inputArea.addEventListener("keydown", (e) =>
			this.handleInputAreaKeydown(e, scaleY),
		);
	}
	...
	private async inputDoneHandler(
		...
		Konva.Image.fromURL(imageUrl, async (image) => {
			// 删除之前的文本节点
			this.getGroupNodesByClassName(
				this.currentShapeGroup.konvaGroup,
				"Text",
			)[0]?.destroy();
			this.currentShapeGroup.konvaGroup.add(image);
			const { width: width_rec, height: height_rec } =
				image.getClientRect();
			image.setAttrs({
				x: pos.x,
				y: pos.y,
			...
			// 修正图像的坐标和尺寸
			const { x, y, width, height } = this.fixImageCoordinateForGroup(
				image,
				this.currentShapeGroup.konvaGroup,
			);
	...
	public async refreshPdfjsAnnotationStorage(
		...
		const image = this.getGroupNodesByClassName(
			ghostGroup,
			"Image",
		)[0] as Konva.Image;
		...
		const annotationStorage = await this.calculateImageForStorage({
			...
			height,
			annotationType:
				rawAnnotationStore.pdfjsAnnotationStorage.annotationType,
	...
	private async calculateImageForStorage({
		...
		const canvasHeight =
			this.konvaStage.size().height / this.konvaStage.scale().y;
