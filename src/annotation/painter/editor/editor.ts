...
export abstract class Editor {
	...
	constructor({
		...
		onAdd,
		editorType,
	}: IEditorOptions & { editorType: AnnotationType }) {
	...
	protected getBgLayer(konvaStage?: Konva.Stage): Konva.Layer {
		return konvaStage
			? konvaStage.getLayers()[0]
			: this.konvaStage.getLayers()[0];
	}
	 ...
	 * @protected
	 */
	protected getNodesByClassName<T extends Konva.Node>(
		className: string,
	): T[] {
		const children = this.currentShapeGroup.konvaGroup.getChildren(
			(node: Konva.Node) => node.getClassName() === className,
		); // 获取当前形状组中指定类名的子节点
	 ...
	 * @protected
	 */
	protected getGroupNodesByClassName<T extends Konva.Node>(
		group: Konva.Group,
		className: string,
	): T[] {
		const children = group.getChildren(
			(node: Konva.Node) => node.getClassName() === className,
		); // 获取指定 Konva.Group 中指定类名的子节点
		return children as unknown as T[]; // 返回子节点数组
	}
	 ...
	 * @protected
	 */
	protected updateKonvaGroup(
		id: string,
		newKonvaGroup: Konva.Group,
	): IShapeGroup | null {
	 ...
	 */
	public addSerializedGroupToLayer(
	 ...
	 */
	public unload() {
		this.shapeGroupStore.forEach((shapeGroup) => {
			shapeGroup.konvaGroup.destroy();
		});
		this.shapeGroupStore.clear();
	}
}
