 ...
 */
const CustomToolbar = forwardRef<CustomToolbarRef, CustomToolbarProps>(
	function CustomToolbar(props, ref) {
		const [currentAnnotation, setCurrentAnnotation] =
			useState<IAnnotationType | null>(null);
		...
		const handleColorChange = (color: string) => {
			...
			const updatedAnnotation = {
				...currentAnnotation,
				style: { ...currentAnnotation.style, color },
			};
			const updatedAnnotations = annotations.map((annotation) =>
				annotation.type === currentAnnotation.type
					? updatedAnnotation
					: annotation,
			);
		...
		const handleFontSizeChange = (size: number) => {
			...
			const updatedAnnotation = {
				...currentAnnotation,
				style: { ...currentAnnotation.style, fontSize: size },
			};
			const updatedAnnotations = annotations.map((annotation) =>
				annotation.type === currentAnnotation.type
					? updatedAnnotation
				...
				<ul className="buttons">
					...
					>
						<li className={isColorDisabled ? "disabled" : ""}>
							<div className="clickable-icon" aria-label={"颜色"}>
								<PaletteIcon
									style={{
										color: currentAnnotation?.style?.color,
									}}
								/>
							</div>
						</li>
					...
					>
						<li className={isFontSizeDisabled ? "disabled" : ""}>
							<div className="clickable-icon" aria-label={"字号"}>
								<FontSizeIcon />
							</div>
						</li>
					</Dropdown>
