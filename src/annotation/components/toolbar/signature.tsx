...
import {
	...
	IAnnotationType,
} from "../../const/definitions"; // 导入自定义类型和默认设置
...
const SignatureTool: React.FC<SignatureToolProps> = (props) => {
	...
	const [currentColor, setCurrentColor] = useState(
		DefaultSignatureSetting.COLORS[0],
	); // 当前选择的颜色
	...
	const changeColor = (color: string) => {
		...
		const allLine =
			konvaStageRef.current
				?.getLayers()[0]
				.getChildren(
					(node: Konva.Node) => node.getClassName() === "Line",
				) || [];
	...
	return (
		<>
			...
			>
				<>
					<div
						...
						aria-label={props.annotation.name}
					>
			...
			>
				<div>
					<div
						className="SignatureTool-Container"
						style={{ width: DefaultSignatureSetting.WIDTH }}
					>
						<div
							ref={containerRef}
							style={{
								height: DefaultSignatureSetting.HEIGHT,
								width: DefaultSignatureSetting.WIDTH,
							}}
						></div>
					</div>
					<div
						className="SignatureTool-Toolbar"
						style={{ width: DefaultSignatureSetting.WIDTH }}
					>
						<div className="colorPalette">
							{DefaultSignatureSetting.COLORS.map((color) => (
								<div
									...
									className={`cell ${color === currentColor ? "active" : ""}`}
									key={color}
								>
									<span
